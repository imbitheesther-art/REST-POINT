...
app.get('/health', (req, res) => {
  res.status(200).json({
    status: 'UP',
    service: 'bodycheckout-service',
    tenant: req.tenantSlug,
    timestamp: new Date().toISOString()
  });
});

app.get('/api/v1/restpoint/bodycheckout', async (req, res) => {
  res.json({
    success: true,
    message: 'Hello from bodycheckout-service!',
    tenant: req.tenantSlug
  });
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`bodycheckout-service is running on port ${PORT}`);
});
