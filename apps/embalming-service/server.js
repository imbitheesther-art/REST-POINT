...
const embalmingRoutes = require('../routes/embalming');
...
app.get('/health', (req, res) => {
  res.status(200).json({
    status: 'UP',
    service: 'embalming-service',
    tenant: req.tenantSlug,
    timestamp: new Date().toISOString()
  });
});

app.use('/api/v1/restpoint', embalmingRoutes);

app.listen(PORT, '0.0.0.0', () => {
  console.log(`embalming-service is running on port ${PORT}`);
});
